...
RSpec.describe NotificationService, type: :service do
  ...
  let(:instance) { described_class.new(push_user, request, current_user) }

  before { create(:established_connection, creator: sender, target: receiver) }

  describe '#send_video_received_notification' do
    let(:user) { receiver }
    let(:params) do
    ...
    subject { instance.send_video_received_notification(params, sender.mkey, sender.first_name, video_id) }
    ...
    describe 'event notification' do
      ...
      subject do
        ...
        instance.send_video_received_notification(params, sender.mkey, sender.first_name, video_id)
      end
  ...
  describe '#send_video_status_updated_notification' do
    let(:user) { sender }
    let(:params) do
    ...
    subject { instance.send_video_status_updated_notification(params) }
    ...
    describe 'event notification' do
      ...
      subject do
        allow(GenericPushNotification).to receive(:send_notification)
        instance.send_video_status_updated_notification(params)
      end
